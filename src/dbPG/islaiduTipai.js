...
async function getOne(id) {
  ...
  try {
    ...
    const { rows } = await query(
      conn,
      "select id, pavadinimas from islaidu_tipai where id = $1",
      [id],
    );
    if (rows.length > 0) {
      return rows[0];
...
async function insert(pavadinimas) {
  ...
  try {
    ...
    const { rowCount } = await query(
      conn,
      "insert into islaidu_tipai (pavadinimas) values ($1)",
      [pavadinimas],
    );
    if (rowCount > 0) {
      const { rows } = await query(
        conn,
        `select currval('islaidu_tipai_id_seq') as "insertId";`
      )
...
async function update(id, pavadinimas) {
  ...
  try {
    ...
    const { rowCount } = await query(
      conn,
      "update islaidu_tipai set pavadinimas = $1 where id = $2",
      [pavadinimas, id],
    );
    if (rowCount > 0) {
      tx = await commit(conn);
...
async function deleteOne(id) {
  ...
  try {
    ...
    const { rowCount } = await query(
      conn,
      "delete from islaidu_tipai where id = $1",
      [id],
    );
    if (rowCount > 0) {
      tx = await commit(conn);
