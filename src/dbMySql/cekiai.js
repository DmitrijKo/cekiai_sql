...
async function getAll(userId) {
   if (!await checkPermission(userId, "cekiai_select")) {
...
async function getPrekes(userId, cekisId) {
   if (!await checkPermission(userId, "cekiai_select")) {
      throw new Error("Permission denied");
   }
...
async function insert(userId, cekis) {
   if (!await checkPermission(userId, "cekiai_insert")) {
      throw new Error("Permission denied");
   }
   let conn;
   let tx;
...
async function update(userId, cekis) {
   if (!await checkPermission(userId, "cekiai_update")) {
      throw new Error("Permission denied");
   }
...
async function deleteOne(userId, id) {
   if (!await checkPermission(userId, "cekiai_delete")) {
      throw new Error("Permission denied");
   }
   id = parseInt(id);
   if (!isFinite(id)) {
